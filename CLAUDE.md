...
## 项目概述
基于 Python 标准库的 A 股自动分析系统，每天早 6:00（北京时间）自动运行，
把上一个交易日的复盘通过微信推送出去。
...
## GitHub Actions
- `daily-analysis.yml` — 每天 06:00 UTC+8 一推（cron `0 22 * * 0-5` UTC），
  内容是**上一个交易日**的收盘复盘；周六早上这一次负责复盘周五
- `weekly-review.yml` — 每周六 10:00 UTC+8
- 两个 workflow 都锁 `TZ: Asia/Shanghai`：定时在 22:00 UTC 触发，不锁时区的话
  runner 的 UTC 日期还是「昨天」，会和 Python 写出的北京日期对不上
- 归档文件名一律按 **data_date**（K 线最新日期）而不是运行日期命名 ——
  早 6 点跑，运行日期是今天、复盘的是昨天，按运行日期命名会把周五数据写进周一文件，
  周六的周复盘读到的整周信号就整体错位一天。`run_daily.py` 把 data_date 写进
